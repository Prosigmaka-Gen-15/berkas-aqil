...
function SideBar(){
    ...
    <div className="bg-slate-400 w-40 min-h-screen transition-all duration-300 ease-in-out border-r-2">
      <nav className=" flex flex-col justify-start items-center min-h-screen mt-5">
        <div>
          <A className="text-xl flex flex-row items-center gap-2"><BsShopWindow/> ShopCart</A> 
        </div>
        <div className="flex flex-col gap-5 mt-10">
          {menu.map( (val, index) => { 
            return(
            <A key={index}> {val.icon} {val.name}</A>)
            })
          }
        </div>
      </nav>
...
function A(props){
  ...
  return (
  <a {...props} className={`${className} font-bold mx-2 flex flex-row items-center gap-2`}>{children}</a>
  )
}